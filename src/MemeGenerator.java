...
public class MemeGenerator {
	...
	private String populateSampleMemeGrid(BufferedImage image, Scanner keyboard) {
		...
		try {
			...
			//loop through images and add text in different places
			for (int i = 0; i < 6; i++) {
			...
			// call the method to actually get the choice of position based on the grid images
			int choice = positionChoice(keyboard, list);
			return list[choice].getName();
			
		//if exception is thrown unexpectedly, end the program
		} catch (IOException e) {
			e.printStackTrace();
			System.exit(1);
			return "";
	...
	private String drawImages(String data) {
		try {
			...
			g.dispose();
			ImageIO.write(bufferedImage, "JPG", new File("stamp.jpg"));
